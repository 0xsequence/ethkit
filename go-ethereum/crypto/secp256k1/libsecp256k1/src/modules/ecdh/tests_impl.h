...
static void test_ecdh_api(void) {
    ...
    CHECK(ethkit_secp256k1_ec_pubkey_create(CTX, &point, s_one) == 1);
...
static void test_ecdh_generator_basepoint(void) {
    ...
    for (i = 0; i < 2 * COUNT; ++i) {
        ...
        /* compute "explicitly" */
        CHECK(ethkit_secp256k1_ec_pubkey_serialize(CTX, point_ser, &point_ser_len, &point[1], SECP256K1_EC_UNCOMPRESSED) == 1);
        /* compare */
...
static void test_result_basepoint(void) {
    ...
    for (i = 0; i < 2 * COUNT; i++) {
        ...
        CHECK(ethkit_secp256k1_ec_pubkey_create(CTX, &point, s) == 1);
        CHECK(secp256k1_ecdh(CTX, out, &point, s_inv, NULL, NULL) == 1);
        CHECK(secp256k1_memcmp_var(out, out_base, 32) == 0);

        CHECK(ethkit_secp256k1_ec_pubkey_create(CTX, &point, s_inv) == 1);
