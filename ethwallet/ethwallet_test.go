...
func TestWalletSignMessageExistingPrefix(t *testing.T) {
	wallet, err := ethwallet.NewWalletFromMnemonic("dose weasel clever culture letter volume endorse used harvest ripple circle install")
	assert.NoError(t, err)
	...
	// Sign the same message as in test above, but this time we will include
	// the prefix as part of the message. We are testing that wallet.SignMessage()
	// will adhere to EIP191 in both cases.
	message := "hi"
